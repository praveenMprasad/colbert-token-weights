"""ESCI leave-one-out (LOO) importance weight training.

For each query token, compute:
  full_gap = score(all_tokens, pos) - avg(score(all_tokens, neg_i))
  loo_gap  = score(without_token, pos) - avg(score(without_token, neg_i))
  importance = full_gap - loo_gap  (how much gap drops when token removed)

Normalize importances with softmax → training targets for weight head.
Encoder frozen, only weight head trains.
"""
import json
import os
import torch
...
def masked_maxsim(Q, D, q_mask, d_mask):
    """MaxSim score with masks. Returns (B,) scores."""
    sim = torch.bmm(Q, D.transpose(1, 2))  # (B, Lq, Ld)
    sim = sim.masked_fill(~d_mask.unsqueeze(1), float("-inf"))
    ms, _ = sim.max(dim=-1)  # (B, Lq)
    ms = ms * q_mask.float()
    return ms.sum(dim=-1)  # (B,)


def compute_loo_importance(Q, D_pos, d_pos_mask, D_neg_all, d_neg_mask_all,
                           neg_counts, q_mask, temperature=0.3):
    """Compute leave-one-out importance for each query token.

    Args:
        Q: (B, Lq, dim)
        D_pos: (B, Ld, dim)
        d_pos_mask: (B, Ld)
        D_neg_all: (N_total, Ld, dim)
        d_neg_mask_all: (N_total, Ld)
        neg_counts: (B,)
        q_mask: (B, Lq) bool
        temperature: softmax temperature for target sharpness
    Returns:
        target_weights: (B, Lq) normalized importance weights
    """
    B, Lq, dim = Q.shape
    device = Q.device

    # Full score (all tokens) against pos
    full_pos = masked_maxsim(Q, D_pos, q_mask, d_pos_mask)  # (B,)

    # Full score against each negative, averaged
    full_neg_avg = torch.zeros(B, device=device)
    neg_offset = 0
    # Also precompute per-negative scores for LOO
    neg_scores_list = []  # list of (n_i, ) tensors per query
    for i in range(B):
        n = neg_counts[i].item()
        if n == 0:
            neg_scores_list.append(None)
            continue
        D_negs_i = D_neg_all[neg_offset:neg_offset + n]
        d_neg_mask_i = d_neg_mask_all[neg_offset:neg_offset + n]
        Q_i = Q[i].unsqueeze(0).expand(n, -1, -1)
        q_mask_i = q_mask[i].unsqueeze(0).expand(n, -1)
        scores = masked_maxsim(Q_i, D_negs_i, q_mask_i, d_neg_mask_i)  # (n,)
        full_neg_avg[i] = scores.mean()
        neg_scores_list.append((D_negs_i, d_neg_mask_i, n))
        neg_offset += n

    full_gap = full_pos - full_neg_avg  # (B,)

    # LOO: for each token, mask it out and recompute gap
    importance = torch.zeros(B, Lq, device=device)

    for t in range(Lq):
        # Create mask with token t removed
        loo_mask = q_mask.clone()
        loo_mask[:, t] = False

        # Skip if token was already padding
        active = q_mask[:, t]  # (B,) which queries have real token at t
        if not active.any():
            continue

        # LOO score against pos
        loo_pos = masked_maxsim(Q, D_pos, loo_mask, d_pos_mask)  # (B,)

        # LOO score against negatives (averaged)
        loo_neg_avg = torch.zeros(B, device=device)
        neg_offset = 0
        for i in range(B):
            n = neg_counts[i].item()
            if n == 0:
            ...
            d_neg_mask_i = d_neg_mask_all[neg_offset:neg_offset + n]
            Q_i = Q[i].unsqueeze(0).expand(n, -1, -1)
            loo_mask_i = loo_mask[i].unsqueeze(0).expand(n, -1)
            scores = masked_maxsim(Q_i, D_negs_i, loo_mask_i, d_neg_mask_i)
            loo_neg_avg[i] = scores.mean()
            neg_offset += n

        loo_gap = loo_pos - loo_neg_avg  # (B,)
        # Importance = how much gap drops when token removed
        importance[:, t] = (full_gap - loo_gap) * active.float()

    # Clamp negative importance to 0 (token removal helped = not important)
    importance = importance.clamp(min=0)
    importance = importance * q_mask.float()

    # Normalize to target weights
    importance = importance.masked_fill(~q_mask, float("-inf"))
    target_weights = F.softmax(importance / temperature, dim=-1)

    return target_weights, importance


def train_loo(config: ESCIConfig, encoder_path: str, output_dir: str,
              max_steps: int = None, max_rows: int = None,
              eval_every: int = 500, num_eval: int = 50, max_negs: int = 8):
    """Train weight head on LOO importance targets. Encoder frozen."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    use_amp = device.type == "cuda"
    print(f"Device: {device} | fp16: {use_amp}")
    print(f"LOO importance training: max_negs={max_negs}, temp={config.softmax_temperature}")
    ...
    print(f"Steps: {total_steps} | Eval every: {eval_every}")
    ...
    for epoch in range(config.epochs):
        ...
        for batch in pbar:
            ...
            with torch.no_grad():
                ...
                target_weights, imp = compute_loo_importance(
...
def _run_stage_eval(model, config, tokenizer, device,
    ...
    eval_log.append({
        "step": step, "vanilla_mrr": v_mrr, "weighted_mrr": w_mrr,
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="ESCI LOO importance weight training")
    parser.add_argument("--encoder_path", default=None)
    parser.add_argument("--output_dir", default="outputs/esci/loo_weighted")
    ...
    train_loo(config, args.encoder_path, args.output_dir,
